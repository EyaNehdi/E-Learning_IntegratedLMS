  ...
  const handleLikeClick = async (courseId) => {
    ...
    if (userLikedCourseIds.includes(courseId)) {
      Swal.fire({
        icon: "info",
        title: "Already Liked",
        text: "You have already liked this course.",
        timer: 2000,
    ...
    try {
      ...
      // Ajouter le cours à la liste des cours likés par l'utilisateur
      setUserLikedCourseIds((prev) => [...prev, courseId])

      // Envoyer la requête au serveur
      const res = await axios.post(`${import.meta.env.VITE_API_PROXY}/course/like/${courseId}`, {
        userId: userId,
      })
      ...
      // Si la réponse du serveur est différente de notre mise à jour optimiste, corriger
      if (updatedCourse.likes !== likedCourses[courseId]) {
        setLikedCourses((prev) => ({
          ...prev,
      ...
      console.log("Course liked successfully:", updatedCourse)
    } catch (error) {
      ...
      // Retirer le cours de la liste des cours likés par l'utilisateur
      setUserLikedCourseIds((prev) => prev.filter((id) => id !== courseId))

      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to like the course. Please try again.",
        timer: 2000,
        showConfirmButton: false,
      })
    }
  ...
  const handleCourseAccess = async (course, e) => {
    ...
    if (!user || user.balance < course.price) {
      Swal.fire({
        ...
        text: `You need ${course.price} Trelix Coins to unlock this course. Your current balance is ${
          user ? user.balance : 0
        } Coins.`,
                  ...
                  <div className="widget-inner">
                    <ul className="list-unstyled">
                      <li className="mb-2">
                        <button
                          className={`btn ${
                            accessFilter === "unlocked" ? "btn-success" : "btn-outline-success"
                      ...
                      <li className="mb-2">
                        <button
                          className={`btn ${
                            accessFilter === "locked" ? "btn-danger" : "btn-outline-danger"
                          } w-100 text-start d-flex align-items-center justify-content-between`}
                      ...
                      <li>
                        <button
                          className={`btn ${
                            accessFilter === "all" ? "btn-primary" : "btn-outline-primary"
                          } w-100 text-start`}
            ...
            <div className="col-lg-8">
              ...
              <div className="text-center mb-4">
                <button
                  ...
                  } ${buttonClicked ? "clicked" : ""} ${showingRecommendations ? "active" : ""}`}
              ...
              {accessFilter !== "all" && !showingRecommendations && (
                <div
                  className={`alert ${
                    accessFilter === "unlocked" ? "alert-success" : "alert-danger"
