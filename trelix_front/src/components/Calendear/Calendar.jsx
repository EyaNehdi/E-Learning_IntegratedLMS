...
import { PlusCircle, CalendarIcon, ChevronLeft, ChevronRight, X, Edit, Trash2, Check } from "lucide-react"
...
function Calendar() {
  ...
  const [newGoal, setNewGoal] = useState({ title: "", description: "", color: "#0f6cbf" })
  ...
  useEffect(() => {
    const fetchGoals = async () => {
      try {
        ...
        console.warn("Failed to fetch from API. Loading from localStorage...", apiErr)
        ...
        if (savedGoals) {
          try {
            ...
            console.error("Error parsing saved goals from localStorage:", parseErr)
          }
        }
      }
    }
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const examResponse = await axios.get('http://localhost:5000/Exam/getall');
  ...
  const handleUpdateGoal = async () => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/goals/${editingGoal._id}`, updatedGoal)

      const updatedGoals = goals.map((goal) =>
        goal._id === editingGoal._id
          ? { ...goal, title: newGoal.title, description: newGoal.description, color: newGoal.color }
          : goal
      )
  ...
  const toggleGoalCompletion = async (goalId) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/goals/${goalToUpdate._id}`, updatedGoal);
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-10 md:h-24 p-1 border border-gray-100 ${isToday ? "bg-blue-50" : isPast ? "bg-gray-50" : ""
            } hover:bg-gray-50 cursor-pointer relative`}
          onClick={() => setSelectedDate(new Date(year, month, day))}
        >
          <div className="flex justify-between items-start">
            <div
              className={`flex justify-center md:justify-start items-center h-6 w-6 md:mb-1 rounded-full ${isSelected ? "bg-[#0f6cbf] text-white" : isToday ? "bg-blue-100 text-blue-800" : ""
                }`}
            >
              {day}
            </div>
            ...
            <div className="hidden md:block">
              ...
              {getGoalsForDate(date)
                ...
                .map((goal, index) => (
                  <div
                    ...
                    style={{ backgroundColor: `${goal.color}20`, color: goal.color, borderColor: `${goal.color}40` }}
                  >
                    <div className="w-2 h-2 rounded-full mr-1" style={{ backgroundColor: goal.color }}></div>
              ...
              {getEventsForDate(date)
                ...
                .map((event, index) => (
                  <div
                    key={`event-${index}`}
                    className={`text-xs truncate mb-1 px-1 py-0.5 rounded ${getEventColor(event.type)}`}
                  >
                    {event.title}
                  </div>
                ))}
              ...
              {getEventsForDate(date).length + getGoalsForDate(date).length > 2 && (
                <div className="text-xs text-gray-500 pl-1">
                  +{getEventsForDate(date).length + getGoalsForDate(date).length - 2} more
                </div>
              )}
            </div>
          )}
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push(
        <div key={i} className="border-r border-gray-200 last:border-r-0 flex-1 min-w-[100px]">
          <div
            className={`text-center py-2 border-b cursor-pointer ${isSelected ? "bg-[#0f6cbf] text-white" : isToday ? "bg-blue-50" : ""
              }`}
                ...
                {dayGoals
                  ...
                  .map((goal, index) => (
                    <div
                      ...
                      style={{ backgroundColor: `${goal.color}10`, borderColor: `${goal.color}40` }}
                    >
                      <div className="flex items-center justify-between">
                        <div className="font-semibold flex items-center" style={{ color: goal.color }}>
                          <div className="w-2 h-2 rounded-full mr-1" style={{ backgroundColor: goal.color }}></div>
                          <span className={goal.completed ? "line-through" : ""}>Goal</span>
                        </div>
                        <div className="flex space-x-1">
                          ...
                          >
                            <Check size={14} className={goal.completed ? "text-green-500" : "text-gray-400"} />
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                      ...
                      <div
                        className={`font-medium mt-1 ${goal.completed ? "line-through text-gray-500" : ""}`}
                      ...
                      {goal.description && (
                        <div
                          className={`text-xs mt-1 ${goal.completed ? "line-through text-gray-400" : "text-gray-600"}`}
                        >
                          {goal.description}
                        </div>
                      )}
      ...
      <div className="container mx-auto px-4 py-6">
            ...
            <div className="flex items-center space-x-2 mt-2 sm:mt-0">
              ...
              <div className="border-l pl-2 ml-2 hidden sm:block">
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded ml-1 ${view === "week" ? "bg-[#0f6cbf] text-white" : "hover:bg-gray-200"
                    }`}
                >
                  Week
                </button>
          ...
          <div className="p-6">
            ...
            {showGoalForm && (
              <div className="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="font-medium text-blue-800">{editingGoal ? "Edit Goal" : "Add New Goal"}</h3>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label htmlFor="goalColor" className="block text-sm font-medium text-gray-700 mb-1">
                      Color
                    </label>
                  ...
                  <div className="flex justify-end space-x-2 pt-2">
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-md text-white ${newGoal.title.trim() ? "bg-[#0f6cbf] hover:bg-[#0a5699]" : "bg-gray-400 cursor-not-allowed"
                        }`}
                    >
              ...
              <div className="mb-6">
                ...
                <div className="space-y-3">
                  {selectedDateGoals.map((goal) => (
                    ...
                    >
                      <div className="w-2 flex-shrink-0" style={{ backgroundColor: goal.color }}></div>
                      <div className="flex-1 p-3">
                        <div className="flex justify-between items-start">
                          <h4
                            className={`font-medium ${goal.completed ? "line-through text-gray-500" : ""}`}
                            style={{ color: goal.completed ? undefined : goal.color }}
                          >
                            {goal.title}
                          </h4>
                          <div className="flex space-x-1 ml-2">
                            <button
                              ...
                              title={goal.completed ? "Mark as incomplete" : "Mark as complete"}
                            >
                              <Check size={16} className={goal.completed ? "text-green-500" : "text-gray-400"} />
                            </button>
                            <button
                        ...
                        {goal.description && (
                          <p
                            className={`text-sm mt-1 ${goal.completed ? "line-through text-gray-400" : "text-gray-600"}`}
                          >
                            {goal.description}
                          </p>
                        )}
            ...
            {selectedDateEvents.length > 0 ? (
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-800 mb-3">Events</h3>
                {selectedDateEvents
                  .sort((a, b) => a.date - b.date)
                  .map((event) => (
                    <div key={event.id} className="flex">
                      ...
                      <div className="w-1 bg-gray-200 relative">
                        <div
                          className={`absolute w-3 h-3 rounded-full -left-1 top-1.5 border-2 border-white ${event.type === "assignment"
                              ? "bg-orange-500"
                              : event.type === "quiz"
                                ? "bg-purple-500"
                                : event.type === "exam"
              ...
              <div className="text-center py-10">
                ...
                <h3 className="text-lg font-medium">No events or goals for this day</h3>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden mt-6">
          ...
          <div className="p-4 flex flex-wrap gap-2">
            {[
              { type: "goal", label: "Goal", className: "bg-teal-100 text-teal-800 border-teal-200" },
              { type: "assignment", label: "Assignment", className: "bg-orange-100 text-orange-800 border-orange-200" },
              { type: "quiz", label: "Quiz", className: "bg-purple-100 text-purple-800 border-purple-200" },
              { type: "exam", label: "Exam", className: "bg-red-100 text-red-800 border-red-200" },
              { type: "session", label: "Live Session", className: "bg-green-100 text-green-800 border-green-200" },
              { type: "forum", label: "Forum", className: "bg-blue-100 text-blue-800 border-blue-200" },
              { type: "meeting", label: "Meeting", className: "bg-yellow-100 text-yellow-800 border-yellow-200" },
            ].map((item) => (
              <div key={item.type} className={`flex items-center px-2 py-1 rounded ${item.className}`}>
