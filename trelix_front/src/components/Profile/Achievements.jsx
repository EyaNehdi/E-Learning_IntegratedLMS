  ...
  const fetchAchievements = async () => {
    try {
      const response = await axios.get(
        `https://trelix-xj5h.onrender.com/certificates/getProgress?userId=${user._id}`
      );
  ...
  useEffect(() => {
    if (user) {
      const fetchCertificates = async () => {
        try {
          const response = await axios.get(
            `https://trelix-xj5h.onrender.com/certificates/getUserCertificates?userId=${user._id}`
          );
